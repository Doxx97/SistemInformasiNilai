...
<div class="space-y-6">
    ...
    <!-- PANEL KONTROL: STATUS & TOMBOL PUBLISH -->
        ...
        <div>
            ...
            @if($isPublished)
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <p class="text-green-700 font-bold text-sm uppercase">Sudah Diterbitkan</p>
                        <p class="text-xs text-gray-500">Wali murid dapat melihat nilai saat ini.</p>
                    </div>
                </div>
            @else
                <div class="flex items-center gap-2">
                    <span class="inline-flex items-center justify-center w-8 h-8 bg-orange-100 rounded-full">
                        <svg class="w-5 h-5 text-orange-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"></path></svg>
                    </span>
                    <div>
                        <p class="text-orange-700 font-bold text-sm uppercase">Belum Diterbitkan (Draft)</p>
                        <p class="text-xs text-gray-500">Wali murid belum bisa melihat nilai.</p>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div>
            @if($isPublished)
                <form id="form-unpublish" action="{{ route('guru.walikelas.unpublish') }}" method="POST">
                    @csrf
                    {{-- Perhatikan: type="button" dan onclick="konfirmasiTarik()" --}}
            ...
            @else
                <form id="form-publish" action="{{ route('guru.walikelas.publish') }}" method="POST">
                    @csrf
                    {{-- Perhatikan: type="button" dan onclick="konfirmasiTerbit()" --}}
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left border-collapse border border-gray-200">
                <thead class="bg-gray-800 text-white font-bold text-xs uppercase text-center">
                    <tr>
                        <th rowspan="2" class="p-3 border border-gray-600 w-10 bg-gray-900">No</th>
                        <th rowspan="2" class="p-3 border border-gray-600 text-left min-w-[200px] bg-gray-900 sticky left-0 z-10">Nama Siswa</th>
                    ...
                    <tr>
                        @foreach($mapels as $mapel)
                            <th class="p-2 border border-gray-600 w-16 text-[10px] tracking-tighter">{{ $mapel->nama_mapel }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody class="text-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 border-b border-gray-200 transition">
                        <td class="p-2 text-center border border-gray-200 font-medium">{{ $index + 1 }}</td>
                        <td class="p-2 font-medium border border-gray-200 text-gray-900 sticky left-0 bg-white hover:bg-gray-50 z-10 shadow-sm">{{ $siswa->name }}</td>
                        
                        {{-- Loop Nilai Per Mapel --}}
                        @foreach($mapels as $mapel)
                            @php
                            ...
                            @endphp
                            <td class="p-2 text-center border border-gray-200 {{ $warnatext }}">
                                {{ $skor > 0 ? $skor : '-' }}
                            </td>
                        @endforeach
                        ...
                        <td class="p-2 text-center border border-gray-200">
                            <div class="flex justify-center gap-1">
                                <!-- Tombol Input Catatan -->
                                <a href="{{ route('guru.walikelas.catatan', $siswa->id) }}" class="bg-yellow-100 text-yellow-600 p-1.5 rounded hover:bg-yellow-200 transition" title="Tulis Catatan">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>

                                <!-- Tombol Cetak Rapor -->
                                <a href="{{ route('guru.walikelas.rapor', $siswa->id) }}" target="_blank" class="bg-blue-100 text-blue-600 p-1.5 rounded hover:bg-blue-200 transition" title="Cetak Rapor">
...
<script>
    // 1. Fungsi Konfirmasi TERBITKAN
    function konfirmasiTerbit() {
        Swal.fire({
            title: 'Terbitkan Rapor?',
            text: "Setelah diterbitkan, Wali Murid dapat melihat nilai siswa secara langsung.",
            icon: 'question',
    ...
    // 2. Fungsi Konfirmasi TARIK KEMBALI
    function konfirmasiTarik() {
        Swal.fire({
            title: 'Tarik Kembali Rapor?',
            text: "Rapor akan kembali ke status Draft. Wali Murid tidak akan bisa melihat nilai lagi.",
            ...
            if (result.isConfirmed) {
                document.getElementById('form-unpublish').submit();
            }
        })
    }
</script>
